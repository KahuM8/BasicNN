...
public class a2Part1 {
        ...
        public static void main(String[] _ignored) {
                ...
                double[][] initial_hidden_layer_weights = new double[][] {{-0.28, -0.22},
                                {0.08, 0.20}, {-0.30, 0.32}, {0.10, 0.01}};
                ...
                // TODO: Perform a single backpropagation pass using the first instance only. (In
                // other
                // words, train with 1
                // instance for 1 epoch!). Hint: you will need to first get the weights from a
                // forward pass.

                double[][] weights = nn.output_layer_weights;
                double[][] outputs = nn.forward_pass(instances[0]);
                double[][][] error = nn.backward_propagate_error(instances[0], outputs[0],
                                outputs[1], onehot_encoded[0][0]);
                nn.update_weights(error[0], error[1]);

                int epoch = 100;

                nn.train(instances, onehot_encoded[0], epoch);
                ...
                int[] predicted_labels = nn.predict(instances_test);
                double accuracy = nn.getAccuracy(instance1_prediction, predicted_labels);
